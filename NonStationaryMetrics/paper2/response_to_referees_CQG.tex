...
Reading your item~[6] properly while preparing this revision turned up something
that belongs here rather than in the bibliography reply, because it bears on
whether this boundary is an artefact of our formulation. It is not. Giannoni,
...
The restriction does, however, lift in the spherically symmetric sector, and for
a reason worth recording. There the out-of-plane Jacobi field is \emph{generated
...
$K_{\rm tan}$ altogether. So no conjugate point occurs in \emph{any} direction
precisely when $K_t<0$ along the arc and the swept azimuth stays below $\pi$---
and the companion paper proves the latter for the spherically symmetric rail. In
that sector, and in the non-rotating frozen limit, the \emph{conjugate-point}
statement is therefore fully three-dimensional.
...
is precisely the static-observer thrust. So the optical projection is \emph{not} a
harmonic morphism, and the obstruction is the very quantity that makes the rail
cost something. The framework accounts for the conformal \emph{form} of the optical
metric and for nothing beyond it.
...
note that the relevant instrument is his Lemma, which needs no completeness, so the
incompleteness of the optical exterior is not doing any work in the argument. This
...
revised manuscript sets out three levels in section~\msSecClassification, and I
summarise them here.
...
$|\boldsymbol v|=c$---which is exactly the condition $|W|=\hat E$ of
Lemma~\msLemCompact. The surface this revision is organised around is one that has
been crossed experimentally and whose neighbourhood has been measured. Those experiments supply the
