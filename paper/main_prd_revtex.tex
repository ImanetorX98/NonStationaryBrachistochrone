...
This closed form is specific to the \emph{$\tau$-branch}. Being
Riemannian-pure (Sec.~\ref{sec:thakurta}, the gravitomagnetic wind cancels),
its turning problem factorizes as doranTau and reduces to genus~1. The
$t$-branch instead carries the wind: its angular integrand $\mathcal K(r)$ is
\emph{rational} rather than elliptic, so the $t$-brachistochrone admits
\emph{no} closed separatrix of this kind---only the ergosphere-crossing
threshold $J_+^{t}=\bar P_e/(2a)$ of Eq.~\eqref{eq:Jplus-t}. The
Weierstrass/Doran separatrix is thus a genuinely $\tau$-branch object.
