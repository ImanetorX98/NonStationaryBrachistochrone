...
vector), and Thakurta--Kerr (conformal rotating black hole). Main results:
...
\begin{figure}[tbp]
    ...
    wind); Vaidya an ingoing radial wind ($f-E^2<0$, driven by $m(v)$);
    Thakurta--Kerr an angular wind (frame dragging), the ellipse shrinking
    with $A(\eta)$.}
...
Schwarzschild limit. \emph{Penetration:} the $\tau$-brachistochrone reaches the
apparent horizon $r=2m(v)$ only if $|J|$ lies below a threshold $J_c(v_0)$;
accretion ($m'>0$) opens a \emph{finite} window $J_c>0$, whereas evaporation
($m'<0$) closes it to zero measure---an
accretion/evaporation asymmetry with no static analogue. \emph{Timing:} because
$m$ grows along the path, $r_{\min}$ depends on the \emph{arrival} epoch $v_1$;
later arrivals meet a heavier hole and turn earlier. \emph{Bounce:} in the
$v$-parametrization the periapsis is a regular bounce, with the memory term
$p_v(r)\propto m'$ measuring the departure from the frozen-mass curve
(Fig.~\ref{fig:vaidya-traj}).
...
% ======================================================================
\section{Thakurta--Kerr: conformal rotating black hole}
\label{sec:thakurta}
% ----------------------------------------------------------------------
Thakurta--Kerr is conformal Kerr, $g=A(\eta)^2 g_{\rm Kerr}$
\cite{Thakurta1981,Kerr1963}, one of a family of cosmological black holes
...
conformal-time and proper-time Hamiltonians are
\begin{align}
...
$\tilde p_\varphi=p_\varphi-2MsA^2/(r\Ehat)$. Both are regular through the
...
\begin{figure}[tbp]
    ...
    \caption{Separatrix $J=\Jc$: ODE, quadrature and evaluated Weierstrass
    agree to $2.8\times10^{-10}$; the PMP-$\eta$ terminates at $\re$.}
    \label{fig:sep3}
\end{figure}
...
fixed-endpoint constraint. Scanning the black-hole spin over the entire
...
black hole breaks the $t/\tau$ symmetry through the gravitational gradient. On
...
\paragraph{Equatorial separatrix in Weierstrass functions.} On the separatrix
$J=\Jc$ the doranTau factorization $\Delta-\Jc^2 w=f(r^2+c^2)$, $c=a/E$, makes the
...
\begin{gather}
    A=\frac{Ma^2}{2E^2},\qquad B=\frac{a^2(E^2-1)}{12E^2},\qquad
...
=\int_0^r\dd r'/\sqrt{Q_4}$. With $r_\pm=M\pm\sqrt{M^2-a^2}$ the horizons,
\begin{equation}
    \alpha_\pm=\frac{r_\pm\big((E^2-1)r_\pm+2M\big)}{r_\pm-r_\mp},\quad
    \lambda_\pm=\frac{\alpha_\pm}{\sqrt{Q_4(r_\pm)}},\quad v_\pm=z(r_\pm),
\end{equation}
$\wp(v_\pm)=A/r_\pm+B$, and
$\Lambda_0=(E^2-1)-\alpha_+/r_+-\alpha_-/r_-+2\lambda_+\zeta(v_+)+2\lambda_-\zeta(v_-)$,
the angle is linear in $z$ plus two Weierstrass-$\sigma$ ratios,
\begin{equation}
    \varphi(r)=\frac{a}{E}\!\left[\Lambda_0\,z
      +\lambda_+\ln\frac{\sigma(z-v_+)}{\sigma(z+v_+)}
      +\lambda_-\ln\frac{\sigma(z-v_-)}{\sigma(z+v_-)}\right]+\text{const}.
    \label{eq:sep-phi}
\end{equation}
...
``NonStationaryBrachistochrone''~\cite{Rosignoli2026Code}:
\begin{center}
\href{https://github.com/ImanetorX98/NonStationaryBrachistochrone}
{\texttt{github.com/ImanetorX98/NonStationaryBrachistochrone}}.
\end{center}
